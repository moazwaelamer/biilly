  ...
  import { CheckCircle, ChevronRight, Copy, Printer, Upload, X } from "lucide-react";
  ...
  export function Booking() {
    ...
    const handleSubmit = async (e: any) => {
      ...
      if (!instapayRef.trim()) { toast.error("Please enter InstaPay reference number"); return }
      if (!transferScreenshot) { toast.error("Please upload transfer screenshot"); return }
      ...
      try {
        ...
        if (!response.ok) { toast.error(data.error || "Booking failed"); return }
    ...
    const handlePrint = () => {
      ...
      if (!win) return
      win.document.write(`
        <html><head><title>Booking Receipt - Billy's Hub</title>
          <style>
        ...
        <body>
          <div class="receipt">
            ...
            <div class="grid">
              <div class="section"><div class="label">${bookingType === "movie" ? "Event" : "Room"}</div><div class="value">${roomName || "Room"}</div></div>
              <div class="section"><div class="label">Type</div><div class="value"><span class="badge">${mode || bookingType}</span></div></div>
              <div class="section"><div class="label">Date</div><div class="value">${formData.date}</div></div>
              <div class="section"><div class="label">From → To</div><div class="value">${formData.time} → ${calcEndTime()}</div></div>
              <div class="section"><div class="label">Duration</div><div class="value">${formData.duration} Hour${Number(formData.duration) > 1 ? "s" : ""}</div></div>
              <div class="section"><div class="label">Price/hr</div><div class="value">${basePrice} EGP</div></div>
            </div>
            <div class="divider"></div>
            <div class="total-row">
            ...
            <div class="divider"></div>
            <div class="section">
              <div class="label">Deposit Paid (50%)</div>
              <div class="value">${depositAmount} EGP (InstaPay)</div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-10 text-center space-y-6">
              ...
              <div ref={receiptRef} className="bg-black/40 border border-white/10 rounded-xl p-5 text-left space-y-3 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><p className="text-gray-400 text-xs">Customer</p><p className="font-bold">{formData.name}</p><p className="text-gray-400 text-xs">{formData.phone}</p></div>
                  <div><p className="text-gray-400 text-xs">{bookingType === "movie" ? "Event" : "Room"}</p><p className="font-bold">{roomName}</p><p className="text-gray-400 text-xs capitalize">{mode || bookingType}</p></div>
                  <div><p className="text-gray-400 text-xs">Date</p><p className="font-bold">{formData.date}</p></div>
                  <div><p className="text-gray-400 text-xs">Time</p><p className="font-bold">{formData.time} → {calcEndTime()}</p></div>
                  <div><p className="text-gray-400 text-xs">Duration</p><p className="font-bold">{formData.duration} hr{Number(formData.duration) > 1 ? "s" : ""}</p></div>
                  <div><p className="text-gray-400 text-xs">Price/hr</p><p className="font-bold">{basePrice} EGP</p></div>
                </div>
                ...
                <div className="border-t border-white/10 pt-3 flex justify-between items-center">
                  <span className="text-gray-400">Deposit Paid (50%)</span>
                  <span className="font-bold">{depositAmount} EGP • {instapayRef}</span>
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={handlePrint} className="flex-1 flex items-center justify-center gap-2 py-3 border border-white/20 hover:border-white/40 rounded-lg text-sm font-bold transition">
                  <Printer className="w-4 h-4" /> Print Receipt
                </button>
                <button onClick={() => navigate("/profile")} className={`flex-1 py-3 ${theme.bg} ${theme.hover} rounded-lg text-sm font-bold transition`}>
                  Go To Profile
                </button>
              </div>
            </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {isLoggedIn && (
                        <div className={`flex items-center gap-3 p-3 rounded-lg border ${theme.border} bg-white/5 mb-5`}>
                          ...
                          <span className={`ml-auto text-xs ${theme.text} border ${theme.border} px-2 py-1 rounded shrink-0`}>✓ Logged in</span>
                        </div>
                      )}
                  ...
                  {step === 3 && (
                    ...
                    >
                      {/* POLICY - ثابت */}
  <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-5 space-y-2 text-xs text-gray-300 leading-relaxed">
    <p className="font-bold text-yellow-400 text-sm mb-2">Booking Policy / سياسة الحجز</p>
    <p><span className="font-semibold text-white">Payment:</span> Confirmed after InstaPay deposit only.</p>
    <p><span className="font-semibold text-white">Cancel 1hr+:</span> Full refund. Within 1hr: 50%. No-show: No refund.</p>
    <hr className="border-white/10" />
    <p><span className="font-semibold text-white">الدفع:</span> مؤكد بعد العربون فقط.</p>
    <p><span className="font-semibold text-white">الإلغاء:</span> قبل ساعة ← كامل. خلال ساعة ← ٥٠٪. عدم حضور ← لا استرداد.</p>
  </div>
                      <div className="bg-white/5 border border-white/10 rounded-xl p-6 space-y-4">
                        ...
                        <div className={`flex items-center justify-between p-4 rounded-lg border ${theme.border} bg-black/20`}>
                          <div>
                            ...
                            <p className="text-2xl font-black tracking-widest">0155 2858668</p>
                            <p className="text-gray-400 text-xs mt-1">Billy's Hub</p>
                          </div>
                          <button type="button" onClick={() => { navigator.clipboard.writeText("0155 2858668"); toast.success("Copied!") }}
                        ...
                        <div className={`p-4 rounded-lg border ${theme.border} bg-black/20 flex justify-between items-center`}>
                          <div>
                            <p className="text-gray-400 text-xs mb-1">Deposit (50%) / العربون</p>
                            <p className={`text-2xl font-black ${theme.text}`}>{depositAmount} EGP</p>
                            <p className="text-gray-400 text-xs mt-1">Total: {finalTotal} EGP (rest paid on arrival)</p>
                          </div>
                          <button type="button" onClick={() => { navigator.clipboard.writeText(depositAmount.toString()); toast.success("Copied!") }}
                            className="flex items-center gap-1 text-xs border border-white/20 px-3 py-2 rounded hover:border-white/40 transition">
                            <Copy className="w-3 h-3" /> Copy
                          </button>
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm text-gray-400">Transfer Screenshot / صورة التحويل <span className="text-red-400">*</span></label>
                          {screenshotPreview ? (
                            <div className="relative">
                              <img src={screenshotPreview} className="w-full rounded-lg border border-white/10 max-h-48 object-contain bg-black/20" />
                              <button type="button" onClick={() => { setTransferScreenshot(null); setScreenshotPreview(null) }}
                        ...
                        <div className="space-y-2">
                          <label className="text-sm text-gray-400">Reference Number / رقم المرجع <span className="text-red-400">*</span></label>
                          <input placeholder="Enter reference number / ادخل رقم المرجع" value={instapayRef}
              ...
              <div className="bg-white/5 border border-white/10 p-6 rounded-xl space-y-4">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center bg-white/5 border border-white/10 rounded-lg px-3 py-2">
                    <div>
                      <p className="font-bold text-sm">Deposit (50%)</p>
                      <p className="text-gray-400 text-xs">Pay now via InstaPay</p>
                    </div>
                    <span className={`font-black text-lg ${theme.text}`}>{depositAmount} EGP</span>
                  </div>
